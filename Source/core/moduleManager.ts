...
namespace AMDLoader {
	...
	export class Module {
		...
		private static _safeInvokeFunction(
			callback: Function,
			args: any[],
		): { returnedValue: any; producedError: any } {
			try {
				return {
		...
		public complete(
			...
			config: Configuration,
			dependenciesValues: any[],
			inversedependenciesProvider: (moduleId: number) => string[],
		): void {
				...
				if (typeof this._callback === "function") {
					...
					if (
						...
						(!this.exportsPassedIn ||
							Utilities.isEmpty(this.exports))
					) {
	...
	export class ModuleManager {
		...
		public getRecorder(): ILoaderEventRecorder {
			if (!this._recorder) {
				if (this._config.shouldRecordStats()) {
					this._recorder = new LoaderEventRecorder(
						this._loaderAvailableTimestamp,
					);
				} else {
		 ...
		 */
		public enqueueDefineAnonymousModule(
			dependencies: string[],
			callback: any,
		): void {
		...
		public defineModule(
			...
			errorback: ((err: AnnotatedError) => void) | null | undefined,
			stack: string | null,
			moduleIdResolver: ModuleIdResolver = new ModuleIdResolver(
				strModuleId,
			),
		): void {
		...
		private _relativeRequire(
			...
			callback?: Function,
			errorback?: (err: AnnotatedError) => void,
		): any {
		...
		private _createRequire(
			...
			let result: IRelativeRequire = <any>((
				dependencies: any,
				callback?: Function,
				errorback?: (err: AnnotatedError) => void,
			) => {
			...
			if (dependencies) {
				for (let i = 0, len = dependencies.length; i < len; i++) {
					...
					if (dependency instanceof PluginDependency) {
						...
						let inversePluginDeps: PluginDependency[] | undefined =
